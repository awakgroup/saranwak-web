...
// .wrangler/tmp/pages-1xO3cP/functionsWorker-0.7862958940032833.mjs
...
var onRequestGet = /* @__PURE__ */ __name2(async (context) => {
  try {
    ...
    const [categoriesResult, tagsResult, placesResult, galleriesResult] = await Promise.all([
    ...
    const galleriesByPlaceId = /* @__PURE__ */ new Map();
...
var onRequestPost3 = /* @__PURE__ */ __name2(async (context) => {
  try {
    ...
    return json(
      {
        success: false,
        message: error instanceof Error ? error.message : "Gagal menambahkan tempat."
      },
      500
    );
  }
...
var onRequestPatch = /* @__PURE__ */ __name2(async (context) => {
  try {
    ...
    const existing = await context.env.DB.prepare("SELECT id FROM places WHERE id = ?").bind(id).first();
    if (!existing) {
      return json(
        {
          success: false,
    ...
    return json(
      {
        success: false,
        message: error instanceof Error ? error.message : "Gagal memperbarui tempat."
      },
      500
    );
  }
...
var onRequestDelete = /* @__PURE__ */ __name2(async (context) => {
  try {
    ...
    return json(
      {
        success: false,
        message: error instanceof Error ? error.message : "Gagal menghapus tempat."
      },
      500
    );
  }
}, "onRequestDelete");
async function getPlaceTagsByPlaceId(db, placeIds) {
...
__name(getPlaceTagsByPlaceId, "getPlaceTagsByPlaceId");
__name2(getPlaceTagsByPlaceId, "getPlaceTagsByPlaceId");
async function replacePlaceTags(db, placeId, tagIds) {
  ...
  for (const tagId of uniqueTagIds) {
    await db.prepare(
      ...
      VALUES (?, ?)
    `
    ).bind(placeId, tagId).run();
  }
}
__name(replacePlaceTags, "replacePlaceTags");
__name2(replacePlaceTags, "replacePlaceTags");
async function replaceGalleries(db, placeId, photoUrls, placeName) {
...
__name(replaceGalleries, "replaceGalleries");
__name2(replaceGalleries, "replaceGalleries");
...
function isAdminAuthenticated(request) {
  ...
  return cookie.split(";").map((item) => item.trim()).some((item) => item === "saranwak_admin_session=active");
}
__name(isAdminAuthenticated, "isAdminAuthenticated");
__name2(isAdminAuthenticated, "isAdminAuthenticated");
...
__name(nullableString, "nullableString");
__name2(nullableString, "nullableString");
...
__name2(nullableNumber, "nullableNumber");
...
__name(toSqlBool, "toSqlBool");
__name2(toSqlBool, "toSqlBool");
...
var onRequestGet2 = /* @__PURE__ */ __name2(async (context) => {
  try {
    ...
    const galleries = await context.env.DB.prepare(
      `
        SELECT 
          id,
          image_url,
...
var onRequestGet3 = /* @__PURE__ */ __name2(async (context) => {
  try {
    ...
    const placesResult = await context.env.DB.prepare(placesQuery).bind(...params).all();
    const places = placesResult.results ?? [];
    if (places.length === 0) {
      return publicJson({
        success: true,
...
var onRequestHead = /* @__PURE__ */ __name2(async () => {
  return new Response(null, {
    status: 200,
    headers: getPublicCacheHeaders()
  });
}, "onRequestHead");
async function getTagsByPlaceId(db, placeIds) {
  const tagsByPlaceId = /* @__PURE__ */ new Map();
  if (placeIds.length === 0) {
...
__name(getTagsByPlaceId, "getTagsByPlaceId");
__name2(getTagsByPlaceId, "getTagsByPlaceId");
...
__name(getPublicCacheHeaders, "getPublicCacheHeaders");
__name2(getPublicCacheHeaders, "getPublicCacheHeaders");
...
__name(publicJson, "publicJson");
__name2(publicJson, "publicJson");
function errorJson(body, status = 500) {
  return Response.json(body, {
    status,
    headers: {
      "Cache-Control": "no-store",
...
// .wrangler/tmp/bundle-cY4nde/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-cY4nde/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.7862958940032833.js.map
